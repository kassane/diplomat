...
pub fn gen_result<W: fmt::Write>(
    ...
    if let ast::TypeName::Result(ok, err, _) = typ {
        ...
        writeln!(out, "pub const {result_name} = extern struct {{")?;
        ...
        if !ok.is_zst() || !err.is_zst() {
            writeln!(&mut result_indent, "{result_name}: union {{")?;
            let mut union_indent = indented(&mut result_indent).with_str("    ");

            if !ok.is_zst() {
                write!(&mut union_indent, "ok: ")?;
                gen_type(
                    ok,
                    in_path,
                    env,
                ...
                writeln!(&mut union_indent, ",")?;
            }

            if !err.is_zst() {
                write!(&mut union_indent, "err: ")?;
                gen_type(
                    err,
                    in_path,
                    env,
        ...
        writeln!(&mut result_indent, "is_ok: bool,")?;
        writeln!(out, "}};")?;
