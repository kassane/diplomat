...
pub static RUNTIME_ZIG: &str = include_str!("runtime.zig");

pub fn gen_bindings(env: &Env, outs: &mut HashMap<String, String>) -> fmt::Result {
    let diplomat_runtime_out = outs.entry("diplomat_runtime.zig".to_string()).or_default();
    write!(diplomat_runtime_out, "{RUNTIME_ZIG}")?;
...
fn gen_struct_header<'a>(
    ...
    let out = outs.entry(format!("{}.zig", typ.name())).or_default();

    writeln!(out, "//! {} generated by diplomat", typ.name())?;
    writeln!(out, "const std = @import(\"std\");")?;
    writeln!(out, "pub const diplomat_runtime = @import(\"diplomat_runtime.zig\");")?;
    writeln!(out)?;

    let mut seen_includes = HashSet::new();
    seen_includes.insert(format!("const {} = @import(\"{}.zig\");", typ.name(), typ.name()));
    seen_includes.insert(format!("pub const {} = opaque{{}};", typ.name()));
    ...
    match typ {
        ...
        ast::CustomType::Enum(enm) => {
            writeln!(out)?;
            writeln!(out, "pub const {} = enum {{", enm.name)?;
    ...
    let mut seen_includes = HashSet::new();
    seen_includes.insert(format!("pub const {} = @import(\"{}.zig\");", typ.name(), typ.name()));
    ...
    write!(out, "pub extern fn {}_destroy(self: ", typ.name())?;
    gen_type(
...
fn gen_result_header(
    ...
    if let ast::TypeName::Result(ok, err, _) = typ {
        let out = outs.entry(format!("{}.zig", name_for_type(typ))).or_default();
...
pub fn gen_includes<W: fmt::Write>(
    ...
    match typ {
        ast::TypeName::Named(path_type) | ast::TypeName::SelfType(path_type) => {
            ...
            match (custom_typ, behind_ref) {
                (ast::CustomType::Opaque(_) | ast::CustomType::Struct(_), true) => {
                    ...
                    //     let include = format!("pub const {} = @import(\"{}.zig\");", custom_typ.name(), custom_typ.name());
        ...
        ast::TypeName::Result(_, _, _) => {
            let include = format!("pub const {} = @import(\"{}.zig\");", name_for_type(typ), name_for_type(typ));
            if !seen_includes.contains(&include) {
                writeln!(out, "{include}")?;
                seen_includes.insert(include);
            }
