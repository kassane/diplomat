...
impl<'ccx, 'tcx: 'ccx, 'header> TyGenContext<'ccx, 'tcx, 'header> {
    pub fn gen_enum_def(&mut self, def: &'tcx hir::EnumDef, id: TypeId) {
        ...
        writeln!(self.decl_header, "pub const {ty_name} = enum {{").unwrap();
    ...
    pub fn gen_dtor(&mut self, id: TypeId) {
        ...
        write!(
            self.impl_header,
            "pub extern fn {ty_name}_destroy(self: *{ty_name}) void;\n\n"
        )
        .unwrap();
    }
    ...
    pub fn gen_ty_decl<'a, P: TyPosition>(
        ...
        match ty {
            Type::Slice(hir::Slice::Str(..)) if !is_struct => {
                vec![
                    (format!("{param_name}_data:").into(), "[*:0]const u8".into()),
                    (format!("{param_name}_len:").into(), "usize".into()),
                ]
            }
