...
use super::types::zig_type_for_prim;
...
pub fn gen_struct<W: fmt::Write>(
    ...
    match custom_type {
        ast::CustomType::Opaque(opaque) => {
            writeln!(out, "pub const {} = opaque{{}};", opaque.name)?;
        }

        ast::CustomType::Struct(strct) => {
            write!(out, "pub const {} = struct {{", strct.name)?;
...
pub fn gen_method<W: fmt::Write>(
    ...
    write!(out, "pub extern fn {}(", method.full_path_name)?;
