...
pub static RUNTIME_MODULE: &str = include_str!("runtime.d");

pub fn gen_bindings(env: &Env, outs: &mut HashMap<String, String>) -> fmt::Result {
    let diplomat_runtime_out = outs.entry("diplomat_runtime.d".to_string()).or_default();
    write!(diplomat_runtime_out, "{RUNTIME_MODULE}")?;
...
fn gen_struct_header<'a>(
    ...
    let out = outs.entry(format!("{}.d", typ.name())).or_default();

    writeln!(out, "import std.stdio;")?;
    writeln!(out, "import diplomat_runtime;")?;
    writeln!(out)?;

    let mut seen_includes = HashSet::new();
    seen_includes.insert(format!("import {}", typ.name()));
...
fn gen_result_header(
    ...
    if let ast::TypeName::Result(ok, err, _) = typ {
        let out = outs.entry(format!("{}.d", name_for_type(typ))).or_default();
...
pub fn gen_includes<W: fmt::Write>(
    ...
    match typ {
            ...
            match (custom_typ, behind_ref) {
                (ast::CustomType::Opaque(_) | ast::CustomType::Struct(_), true) => {
                    if pre_struct {
                        ...
                        let include = format!("import {};", custom_typ.name());
        ...
        ast::TypeName::Result(_, _, _) => {
            let include = format!("import {};", name_for_type(typ));
            if !seen_includes.contains(&include) {
                writeln!(out, "{include}")?;
                seen_includes.insert(include);
            }
